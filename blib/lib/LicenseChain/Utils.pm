...
sub verify_webhook_signature {
    ...
    return 0 if !defined($payload) || !defined($signature) || !defined($secret) || $payload eq '' || $secret eq '';
    my $expected = create_webhook_signature($payload, $secret);
    my $received = ($signature =~ /^sha256=(.*)\z/s) ? $1 : $signature;
    return 0 if length($expected) != length($received);
    my $diff = 0;
    for my $i (0 .. length($expected) - 1) {
        $diff |= ord(substr($expected, $i, 1)) ^ ord(substr($received, $i, 1));
    }
    return $diff == 0;
}
