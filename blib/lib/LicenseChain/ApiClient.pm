...
sub patch {
    my ($self, $endpoint, $data) = @_;
    return $self->make_request('PATCH', $endpoint, $data);
}
...
sub build_url {
    my ($self, $endpoint, $params) = @_;
    my $normalized_endpoint = $endpoint;
    if ($normalized_endpoint !~ m{^/v1/}) {
        if ($normalized_endpoint =~ m{^/}) {
            $normalized_endpoint = '/v1' . $normalized_endpoint;
        } else {
            $normalized_endpoint = '/v1/' . $normalized_endpoint;
        }
    }
    my $base_url = $self->{config}->get_base_url();
    $base_url =~ s{/$}{};
    my $url = $base_url . $normalized_endpoint;
    
    if ($params && %$params) {
        my @query_params;
        while (my ($key, $value) = each %$params) {
            push @query_params, "$key=" . uri_escape($value);
        }
